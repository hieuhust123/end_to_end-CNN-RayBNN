...
f_set = os.path.join(HERE, 'file_sets_part2.mat')
...
np.save(os.path.join(predictions_dir, 'features_part2_0.npy'), features)
np.save(os.path.join(predictions_dir, 'labels_part2_0.npy'), labels)
np.save(os.path.join(predictions_dir, 'features_val_part2_0.npy'), features_val)
np.save(os.path.join(predictions_dir, 'labels_val_part2_0.npy'), labels_val)

_elapsed = time.time() - _script_start
print(f"[train_feature_part2.py] Total runtime: {_elapsed:.1f}s ({_elapsed/3600:.2f}h)")
