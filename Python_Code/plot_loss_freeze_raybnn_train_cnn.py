...
def plot_losses(batch_losses, epoch_losses, output_dir="."):
    ...
    if batch_losses:
        ...
        plt.title('Batch-Level Training Loss of model when freeze RayBNN and train CNN(lr=0.001)')
        ...
        plt.legend()
        plt.grid(True)
        batch_plot_filename = os.path.join(output_dir, "cnn_batch_freeze_raybnn_train_cnn.png")
    ...
    if epoch_losses:
        ...
        plt.xlabel('Epoch')
        plt.ylabel('Cross Entropy Loss')
        plt.title('Training Loss of model when freeze RayBNN and train CNN(lr=0.001)')
        # Set x-axis ticks to integer values (spacing of 1)
        max_epoch = len(epoch_losses) - 1
        plt.xticks(np.arange(0, max_epoch + 1, 1))
        # Disable offset notation for y-axis to show full loss values
        ax = plt.gca()
        ax.ticklabel_format(useOffset=False, style='plain')
        # Format y-axis to show appropriate precision
        from matplotlib.ticker import FuncFormatter
        ax.yaxis.set_major_formatter(FuncFormatter(lambda x, p: f'{x:.6f}'))
        final_epoch_loss = epoch_losses[-1]
        plt.text(0.02, 0.95, f'Final loss: {final_epoch_loss:.6f}',
                 transform=plt.gca().transAxes, fontsize=10,
                 bbox=dict(facecolor='white', alpha=0.8, edgecolor='none'))
        plt.legend()
        plt.grid(True)
        epoch_plot_filename = os.path.join(output_dir, "plot_freeze_raybnn_train_cnn.png")
        plt.savefig(epoch_plot_filename)
        print(f"Epoch-level loss plot saved to {epoch_plot_filename}")
        plt.close()

def main():
    log_file = "output_freeze_raybnn_train_cnn.txt"
