...
fn main() {
	...
	let fold = args[2].clone().parse::<u64>().unwrap();

	println!("fold {}", fold);
	

	let size_list = vec![8,16,32,64,128,256,512,1024,2048,4096];
	for gg in 0..20
	{

	
	let mut active_size = size_list[gg];
	let hidden_size = active_size;
	...
	let save_filename = format!("optim_CVLOSS_{}.csv",hidden_size);
	...
	let max_neuron_size: u64 = active_size + 3000;
	...
	//{
		...
		let add_neuron_options: add_neuron_option_type = add_neuron_option_type {
			new_active_size: arch_search.neural_network.netdata.active_size-output_size-input_size+2,
			init_connection_num: arch_search.neural_network.netdata.active_size-10,
			input_neuron_con_rad: 0.7*arch_search.neural_network.netdata.sphere_rad,
			hidden_neuron_con_rad: 0.4*arch_search.neural_network.netdata.sphere_rad,
			output_neuron_con_rad: 0.4*arch_search.neural_network.netdata.sphere_rad,
		};
		...
		raybnn::interface::autotransfer_f32::reduce_weights(
			arch_search.neural_network.WColIdx.dims()[0]-(hidden_size*1000), 
			&mut arch_search
		);
		...
		*/
		...
		let v0 = vec![elapsed, totalparam, hidden_size as f32];
		let info = arrayfire::Array::new(&v0, arrayfire::Dim4::new(&[v0.len() as u64, 1, 1, 1]));


		//Save dataset

		input_size = 1400;
		let save_filename = format!("optim_info_{}.csv",hidden_size);
		...
		let save_filename = format!("optim_pred_{}.csv",hidden_size);
		raybnn::export::dataloader_f32::write_arr_to_csv(
			&save_filename,
			&Yhat_arr
		);
		...
		let save_filename = format!("optim_act_{}.csv",hidden_size);
		raybnn::export::dataloader_f32::write_arr_to_csv(
			&save_filename,
			&Y_arr
		);
