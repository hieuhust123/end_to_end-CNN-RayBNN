...
fn main() {
	...
	let arg_input_vec: Vec<u64> =    vec![ 6,     8,    12,     14,      16,    32,   50,    75,   100,    125,   162];
	...
	for ii in 0..arg_input_vec.len()
	{
		...
		*/
		...
		let filename = format!("before_delete_WRowIdxCOO.csv");
		raybnn::export::dataloader_i32::write_arr_to_csv(
			&filename,
			&WRowIdxCOO
		);

		let filename = format!("before_delete_WColIdx.csv");
		raybnn::export::dataloader_i32::write_arr_to_csv(
			&filename,
			&arch_search.neural_network.WColIdx
		);

		let save_filename = format!("before_delete_WValues.csv");
		...
		if ii == 10
		{
			let del_num = (0.05f64*(WValues.dims()[0] as f64)) as u64;
			raybnn::graph::adjacency_f32::delete_weights_with_prob(
				&mut WValues,
				&mut WRowIdxCOO,
			...
			let filename = format!("after_delete_WRowIdxCOO.csv");
			raybnn::export::dataloader_i32::write_arr_to_csv(
				&filename,
				&WRowIdxCOO
			);
	
			let filename = format!("after_delete_WColIdx.csv");
			raybnn::export::dataloader_i32::write_arr_to_csv(
				&filename,
				&arch_search.neural_network.WColIdx
			);	

			let save_filename = format!("after_delete_WValues.csv");
			raybnn::export::dataloader_f32::write_arr_to_csv(
				&save_filename,
			&WValues
			);
