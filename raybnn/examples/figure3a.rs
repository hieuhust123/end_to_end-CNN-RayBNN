...
fn main() {
	...
	//Run the code 11 times
	//The first run is slower because it has to compile CUDA/OpenCL kernels
	for run_num in 0..11
	{
	...
	for ii in 0..arg_input_vec.len()
	{
		...
		*/
		...
		//Save dataset
		let save_filename = format!("fig3a_info_{}_{}_{}.csv",RT3_RAD as u64,input_size, run_num);
		...
		let save_filename = format!("fig3a_test_pred_{}_{}_{}.csv",RT3_RAD as u64,input_size, run_num);
		raybnn::export::dataloader_f32::write_arr_to_csv(
			&save_filename,
			&Yhat_arr
		);
		...
		let save_filename = format!("fig3a_test_act_{}_{}_{}.csv",RT3_RAD as u64,input_size, run_num);
		raybnn::export::dataloader_f32::write_arr_to_csv(
			&save_filename,
			&Y_arr
		);
