...
use crate::graph::adjacency_f32::select_values;
...
)
{
	...
	for i in 0..keys_size
	{
		...
		if (valsel.dims()[0] == 0)
		{
			...
			let insert_values = arrayfire::constant::<f32>(zero,values_dims);
			...
			let cols_rand = arrayfire::randu::<f32>(cols_dims); 
		...
		let new_values = arrayfire::constant::<f32>(zero,values_dims);
		...
		let mut cols_rand = arrayfire::randu::<f32>(cols_dims); 

		let inarr = arrayfire::constant::<f32>(zero, cvec.dims());
...
)
{
	...
	for i in 0..total_num
	{
		...
		let seqs = &[arrayfire::Seq::new(dist_start as f32, last_idx as f32, 1.0 as f32), ];
...
)
{
	...
	if (dist_start > 0)
	{
		...
		let seqs = &[arrayfire::Seq::new(0.0 as f32, (dist_start-1) as f32, 1.0 as f32) ];
	...
	let seqs = &[arrayfire::Seq::new((last_idx+1) as f32, (WValues_len-1) as f32, 1.0 as f32) ];
	arrayfire::assign_seq(WValues, seqs, &newWValues);
}
...
pub fn sum_and_distribute_UAF(
	...
	alpha0: f32,
	alpha1: f32,
	gH: &arrayfire::Array<f32>,
	gA: &arrayfire::Array<f32>,
	gB: &arrayfire::Array<f32>,
	gC: &arrayfire::Array<f32>,
	gD: &arrayfire::Array<f32>,
	gE: &arrayfire::Array<f32>,
...
)
{
	...
	let mut newH = arrayfire::constant::<f32>(0.0,temp_dims);
	let mut newA = arrayfire::constant::<f32>(0.0,temp_dims);
	let mut newB = arrayfire::constant::<f32>(0.0,temp_dims);
	let mut newC = arrayfire::constant::<f32>(0.0,temp_dims);
	let mut newD = arrayfire::constant::<f32>(0.0,temp_dims);
	let mut newE = arrayfire::constant::<f32>(0.0,temp_dims);
	...
	let seqs = &[arrayfire::Seq::new((last_idx+1) as f32, (H_len-1) as f32, 1.0 as f32) ];
	arrayfire::assign_seq(H, seqs, &newH);
	arrayfire::assign_seq(A, seqs, &newA);
	arrayfire::assign_seq(B, seqs, &newB);
	arrayfire::assign_seq(C, seqs, &newC);
