...
params = cumsum(params);
params = mean(params,2);

%errorbar(input_size_list,MAE_mean,MAE_std,'Color',colorlist(i));
plot(input_size_list,params,'LineWidth',2,'MarkerSize',10);

xlim([0,170]);
ylim([100,1e7]);
set(gca,'YScale','log','Box','on','FontSize',fontsz);

%gca = hINT.Parent;   % Important
% set(gca, 'XTick', input_size_list)

neural_network_list = {'\color{red}\bf RayBNN','CNN','GCN2','LSTM','MLP','GCN2LSTM','BILSTM'};

legend(neural_network_list ,'Location','SouthEast','FontSize',fontsz,'Box','off','NumColumns',2);
xlabel('Num. APs','FontSize',fontsz);
ylabel('Trainable Parameters','FontSize',fontsz)
hold off
drawnow;

saveas(gcf,'RSSIParam','fig');
saveas(gcf,'RSSIParam','png');
