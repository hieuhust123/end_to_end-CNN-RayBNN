...
#metrics = np.loadtxt("metrics.txt", delimiter=",")
...
#n_components = int(metrics[fold,0])
#print(n_components)
#print(metrics[:,0])
...
for cv_index, (train_index, test_index) in enumerate(kf.split(cv_set)):
    ...
    n_components = 11
    clf = make_pipeline(XdawnCovariances(n_components), TangentSpace(metric='riemann'))
    ...
    outdata = np.concatenate((outdata, X_train), axis=1)
    ...
    X_test = clf.transform(X_test[200:])
    outdata = np.concatenate((outdata, X_test), axis=1)
    ...
    outdata = np.concatenate((outdata, X_val), axis=1)
